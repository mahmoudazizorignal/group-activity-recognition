...
class B4ModelProvider(BaselinesInterface):
    ...
    def forward(self, 
        ...
        x2, (_, _) = self.lstm(x1) # (BATCH_SIZE, FRAME_CNT, NO_LSTM_HIDDEN_UNITS2)
        x = torch.concat([x1, x2], dim=2) # (BATCH_SIZE, FRAME_CNT, 2048 + NO_LSTM_HIDDEN_UNITS2)
        x = x[:, -1, :] # (BATCH_SIZE, 2048 + NO_LSTM_HIDDEN_UNITS2)
